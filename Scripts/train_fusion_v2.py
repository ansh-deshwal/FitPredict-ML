...
train_fusion_v2.py
==================
Multi-modal fusion with a dedicated StructureEncoder.

Improvement over train_fusion.py
---------------------------------
* StructureEncoder: learns a 64-d non-linear projection of the 11 raw
  structural features before fusion, instead of concatenating them raw.
  This gives the model capacity to re-weight and combine structural
  signals before they interact with the 1280-d sequence embedding.
* Normalisation is fitted on the training split only (no leakage into
  val / test).
...
Val set : drives ReduceLROnPlateau + early stopping; never seen by model
Test set: used only for final evaluation after best checkpoint is loaded
Outputs : Results/fusion_v2_plot.png
          Results/fusion_v2_predictions.csv
          Models/best_fusion_v2.pt
"""
...
assert S.shape[1] == 11, (
    f"Expected 11 structural features, got {S.shape[1]}. "
    "Re-run extract_structure_features.py."
)
STRUCT_DIM = S.shape[1]  # 11 — read at runtime
...
# Fit normalisation on train only; apply the same shift/scale to val and test
...
X_train_t, S_train_t, y_train_t = to_t(X_train), to_t(S_train), to_t(y_train)
X_val_t,   S_val_t,   y_val_t   = to_t(X_val),   to_t(S_val),   to_t(y_val)
X_test_t,  S_test_t,  y_test_t  = to_t(X_test),  to_t(S_test),  to_t(y_test)
...
class StructureEncoder(nn.Module):
    """
    Learns a 64-d representation of the 11 raw structural features.
    Gives the model capacity to re-weight and combine burial depth,
    secondary structure, rASA, dihedral angles, and contact count
    before they interact with the sequence embedding.
    """
    ...
    def forward(self, s):
        return self.net(s)  # (batch, out_dim)


class MultiModalFusionModel(nn.Module):
    """
    Fuses ESM-2 sequence embeddings (1280-d) with learned structural
    features (11 → 64-d via StructureEncoder).

    Input dim : 1280 + 64 = 1344
    Hidden    : 512  (residual blocks)
    Output    : 1 (DMS fitness score)
    """
    def __init__(self, seq_dim=1280, struct_dim=11,
        ...
        self.projection = nn.Sequential(
            nn.Linear(seq_dim + struct_enc_dim, hidden_dim),
    ...
    def forward(self, seq_emb, struct_feat):
        ...
        x = torch.cat([seq_emb, s], dim=-1)     # (batch, 1344)
...
model = MultiModalFusionModel(struct_dim=STRUCT_DIM).to(device)
print("\nModel Architecture:")
print(model)
...
EPOCHS          = 100
BATCH_SIZE      = 32
NOISE_STD       = 0.01   # Gaussian noise on sequence embeddings (augmentation)
PATIENCE        = 15     # early-stopping patience (val Spearman ρ)
MODEL_SAVE_PATH = MODELS_DIR / "best_fusion_v2.pt"
...
# ── Training loop ──────────────────────────────────────────────────────────────
print("\n" + "=" * 60)
print("Training  —  seq(1280)  +  struct(11→64)")
print("=" * 60)
...
for epoch in range(EPOCHS):
    ...
    for i in range(0, X_train_t.size(0), BATCH_SIZE):
        ...
        bs, by = S_train_t[idx], y_train_t[idx]
...
model.eval()
with torch.no_grad():
    train_preds = model(X_train_t, S_train_t).cpu().numpy()
    test_preds  = model(X_test_t,  S_test_t).cpu().numpy()
...
fig.suptitle(
    f"Fusion v2  —  ESM-2 (1280-d) + StructureEncoder (11→64-d)  "
...
plt.tight_layout()
plot_path = RESULTS_DIR / "fusion_v2_plot.png"
...
}).to_csv(RESULTS_DIR / "fusion_v2_predictions.csv", index=False)
print("Saved predictions → Results/fusion_v2_predictions.csv")
